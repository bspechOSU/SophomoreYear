        ...
        switch (this.kind()) {
            case BLOCK: {
                ...
                for (int i = 0; i < this.lengthOfBlock(); i++) {
                    Statement q = this.removeFromBlock(0);
                    q.prettyPrint(out, offset);
                    this.addToBlock(this.lengthOfBlock(), q);
                }
            ...
            case IF: {
                ...
                Statement m = this.newInstance();
                Condition l = this.disassembleIf(m);
                printSpaces(out, offset);
                out.print("IF ");
                out.print(toStringCondition(l));
                out.println(" THEN");
                Condition j = Condition.NEXT_IS_ENEMY;

                m.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(l, m);
            ...
            case IF_ELSE: {
                ...
                Statement m = this.newInstance();
                Statement n = this.newInstance();
                Condition l = this.disassembleIfElse(m, n);
                printSpaces(out, offset);
                out.print("IF ");
                out.print(toStringCondition(l));
                out.println(" THEN");
                ...
                printSpaces(out, offset);
                out.println("ELSE");
                n.prettyPrint(out, offset + 4);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(l, m, n);

                break;
            }
            case WHILE: {

                Statement y = this.newInstance();
                printSpaces(out, offset);
                out.print("WHILE ");

                Condition z = this.disassembleWhile(y);
                out.print(toStringCondition(z));
                out.print(" DO");
                out.println();
                y.prettyPrint(out, 4 + offset);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(z, y);

                break;
            }
            case CALL: {
                printSpaces(out, offset);
                String m = this.disassembleCall();
                out.println(m);
                this.assembleCall(m);
