        ...
        switch (s.kind()) {
            case BLOCK: {
                ...
                for (int index = 0; index < s.lengthOfBlock(); index++) {
                    Statement rem = s.removeFromBlock(index);
                    generateCodeForStatement(rem, context, cp);
                    s.addToBlock(index, rem);
                }
            ...
            case IF_ELSE: {
                ...
                cp.add(cp.length(), dummy);
                cp.replaceEntry(jump, cp.length());
                //set jump to be the new dummy location
                jump = cp.length() - 1;
                generateCodeForStatement(bElse, context, cp);
                //now go back and set the jump location
                cp.replaceEntry(jump, cp.length());
            ...
            case WHILE: {
                ...
                cp.add(cp.length(), dummy);
                generateCodeForStatement(b, context, cp);
                //add the code to jump back up to the top
                cp.add(cp.length(), Instruction.JUMP.byteCode());
                cp.add(cp.length(), top);
                cp.replaceEntry(jump, cp.length());
                s.assembleWhile(c, b);
            ...
            case CALL: {
                ...
                //it is an instruction
                    ...
                    Reporter.assertElseFatalError(context.hasKey(call),
                            "Violation of: Map has key");

                    Map.Pair<String, Statement> instr = context.remove(call);
                    Statement instrBody = instr.value();

                    generateCodeForStatement(instrBody, context, cp);

                    context.add(call, instrBody);
                }
